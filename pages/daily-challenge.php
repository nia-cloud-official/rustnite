...
<div style="animation: fade-in 0.5s ease-out;">
    ...
    <?php if ($submitted && $result): ?>
        <div class="tw-card tw-card-body mb-6" style="border-color:rgba(0,217,90,0.3); background:rgba(0,217,90,0.1); text-align:center; padding:40px;">
            ...
            <p class="text-twitch-muted mb-2">Base: <span class="font-bold text-white">+<?= $result["base_xp"] ?> XP</span></p>
            <?php if ($result["bonus"] > 0): ?>
                <p class="text-twitch-muted mb-2">Bonus: <span class="font-bold" style="color:#00D95A;">+<?= $result["bonus"] ?> XP</span></p>
            <?php endif; ?>
            <p class="text-lg font-bold gradient-text">Total: +<?= $result["xp_earned"] ?> XP!</p>
        ...
        <div class="tw-card">
            <div class="tw-card-body">
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        <h2 class="text-xl font-bold"><?= htmlspecialchars($daily["title"]) ?></h2>
                        <div class="flex items-center gap-3 mt-1">
                            <span class="text-xs px-2 py-1 rounded-full" style="background:<?= $daily["language_color"] ?>20; color:<?= $daily["language_color"] ?>;">
                                <i class="<?= $languages[array_search($daily["language_id"], array_column($languages, "id"))]["icon"] ?? "fas fa-code" ?>"></i>
                                <?= htmlspecialchars($daily["language_name"]) ?>
                            </span>
                            <span class="text-xs px-2 py-1 rounded-full font-medium
                                <?= $daily["difficulty"] === "beginner" ? "bg-green-500/20 text-green-400" : "" ?>
                                <?= $daily["difficulty"] === "intermediate" ? "bg-blue-500/20 text-blue-400" : "" ?>
                                <?= $daily["difficulty"] === "advanced" ? "bg-red-500/20 text-red-400" : "" ?>
                            ...
                            <span class="text-xs px-2 py-1 rounded-full" style="background:rgba(169,112,255,0.1); color:#A970FF;">
                                <i class="fas <?= get_challenge_type_icon($daily["challenge_type"]) ?>"></i>
                                <?= ucfirst($daily["challenge_type"]) ?>
                            </span>
                        </div>
                    </div>
                    <div style="margin-left:auto; text-align:right;">
                        <div class="text-2xl font-bold" style="color:#A970FF;">+<?= $daily["xp_reward"] ?> XP</div>
                        <?php if ($daily["bonus_xp"] > 0): ?>
                            <div class="text-xs" style="color:#00D95A;">+<?= $daily["bonus_xp"] ?> bonus</div>
                        <?php endif; ?>
                    </div>
                </div>

                <div class="mb-6">
                    <p class="text-sm text-twitch-muted mb-4"><?= nl2br(htmlspecialchars($daily["description"] ?? "")) ?></p>
                </div>

                <form method="POST">
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-twitch-muted mb-2">Your Solution</label>
                        <textarea name="code" rows="10" class="w-full p-4 bg-twitch-dark border border-twitch-border rounded-lg text-twitch-text font-mono text-sm focus:outline-none focus:border-twitch-purple" placeholder="Write your code here..." required><?= htmlspecialchars($daily["starter_code"] ?? "") ?></textarea>
                    </div>
