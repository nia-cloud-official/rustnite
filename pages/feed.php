    ...
    if (empty($content)) {
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
            ");
            $stmt->execute([$_SESSION["user_id"], $post_type, $title, $content, $code_snippet ?: null, $language_slug ?: null, $tags ?: null]);
...
if (isset($_GET["like"])) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT id FROM feed_likes WHERE post_id = ? AND user_id = ?");
        $stmt->execute([$post_id, $_SESSION["user_id"]]);
        if (!$stmt->fetch()) {
            $pdo->prepare("INSERT INTO feed_likes (post_id, user_id) VALUES (?, ?)")->execute([$post_id, $_SESSION["user_id"]]);
            $pdo->prepare("UPDATE feed_posts SET likes_count = likes_count + 1 WHERE id = ?")->execute([$post_id]);
        }
    } catch (PDOException $e) {}
...
if (isset($_GET["unlike"])) {
    ...
    try {
        $pdo->prepare("DELETE FROM feed_likes WHERE post_id = ? AND user_id = ?")->execute([$post_id, $_SESSION["user_id"]]);
        $pdo->prepare("UPDATE feed_posts SET likes_count = GREATEST(0, likes_count - 1) WHERE id = ?")->execute([$post_id]);
    } catch (PDOException $e) {}
    header("Location: index.php?page=feed");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_comment"])) {
    ...
    if (!empty($comment)) {
        try {
            $pdo->prepare("INSERT INTO feed_comments (post_id, user_id, content) VALUES (?, ?, ?)")->execute([$post_id, $_SESSION["user_id"], $comment]);
            $pdo->prepare("UPDATE feed_posts SET comments_count = comments_count + 1 WHERE id = ?")->execute([$post_id]);
        } catch (PDOException $e) {}
    }
...
<div style="animation: fade-in 0.5s ease-out;">
    ...
    <!-- Create Post Form -->
    <div id="create-post-form" class="tw-card tw-card-body mb-6" style="display:<?= $post_error || $post_success ? "block" : "none" ?>; border-color: rgba(145,71,255,0.3);">
        ...
        <?php if ($post_error): ?>
            <div style="background:rgba(233,25,123,0.1); border:1px solid rgba(233,25,123,0.2); border-radius:8px; padding:12px; margin-bottom:16px;">
                <span style="color:#E9197B; font-size:13px;"><?= htmlspecialchars($post_error) ?></span>
            </div>
        <?php endif; ?>
        <?php if ($post_success): ?>
            <div style="background:rgba(0,217,90,0.1); border:1px solid rgba(0,217,90,0.2); border-radius:8px; padding:12px; margin-bottom:16px;">
                <span style="color:#00D95A; font-size:13px;"><?= htmlspecialchars($post_success) ?></span>
            </div>
        <?php endif; ?>
                ...
                <div>
                    ...
                    <select name="post_type" class="w-full p-2 bg-twitch-medium border border-twitch-border rounded-lg text-twitch-text">
                        <?php foreach ($post_types as $key => $info): ?>
                            <option value="<?= $key ?>"><i class="fas <?= $info["icon"] ?>"></i> <?= ucfirst($key) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
    ...
    <div class="flex items-center gap-2 mb-6 overflow-x-auto" style="scrollbar-width:none;">
        <a href="?page=feed" class="tw-btn <?= empty($filter_type) ? "tw-btn-primary" : "tw-btn-ghost" ?> tw-btn-sm">
            <i class="fas fa-globe"></i>
            All
        </a>
        <?php foreach ($post_types as $key => $info): ?>
            <a href="?page=feed&type=<?= $key ?>" class="tw-btn <?= $filter_type === $key ? "tw-btn-primary" : "tw-btn-ghost" ?> tw-btn-sm">
    ...
    <?php if (!empty($posts)): ?>
        <?php foreach ($posts as $post):
            ...
            $comments = max($post["comments_count"], $post["comments_count_real"]);
        ?>
            <div class="tw-card mb-4" style="animation: slide-up 0.3s ease-out;">
                <div class="tw-card-body">
                    <div class="flex items-start gap-4">
                        <!-- Type Icon -->
                        <div class="tw-avatar" style="width:40px; height:40px; font-size:14px; background:<?= $type_info["color"] ?>20; color:<?= $type_info["color"] ?>; flex-shrink:0;">
                        ...
                        <div style="flex:1; min-width:0;">
                            ...
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-medium text-sm"><?= htmlspecialchars($post["username"]) ?></span>
                                <span class="text-xs px-1.5 py-0.5 rounded" style="background:<?= $type_info["color"] ?>20; color:<?= $type_info["color"] ?>; font-size:10px;">
                                    <?= ucfirst($post["type"]) ?>
                                </span>
                                <span class="text-xs text-twitch-muted"><?= time_ago($post["created_at"]) ?></span>
                            ...
                            <?php if (!empty($post["title"])): ?>
                                <h3 class="font-bold text-base mb-1"><?= htmlspecialchars($post["title"]) ?></h3>
                            <?php endif; ?>

                            <!-- Content -->
                            <div class="text-sm text-twitch-text mb-3" style="line-height:1.6; white-space:pre-wrap;"><?= htmlspecialchars($post["content"]) ?></div>

                            <!-- Code Snippet -->
                            <?php if (!empty($post["code_snippet"])): ?>
                                <pre class="p-3 bg-twitch-dark rounded-lg text-sm font-mono mb-3 overflow-x-auto" style="border:1px solid #2D2D35;"><code><?= htmlspecialchars($post["code_snippet"]) ?></code></pre>
                            <?php endif; ?>
                            ...
                            <?php if (!empty($post["tags"])): ?>
                                <div class="flex items-center gap-2 mb-3 flex-wrap">
                                    <?php foreach (explode(",", $post["tags"]) as $tag): ?>
                                        <span class="text-xs px-2 py-0.5 rounded-full" style="background:rgba(145,71,255,0.1); color:#A970FF;">#<?= trim(htmlspecialchars($tag)) ?></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="flex items-center gap-4 mt-2 pt-3" style="border-top:1px solid #2D2D35;">
                                <a href="?page=feed&<?= $user_liked ? "unlike" : "like" ?>=<?= $post["id"] ?>" class="flex items-center gap-1 text-sm <?= $user_liked ? "tw-btn-primary" : "text-twitch-muted" ?>" style="text-decoration:none;">
                                    <i class="fas fa-heart <?= $user_liked ? "text-twitch-pink" : "" ?>" style="color:<?= $user_liked ? "#E9197B" : "" ?>;"></i>
                                    <span><?= $likes ?> <?= $likes === 1 ? "like" : "likes" ?></span>
                                </a>

                                <a href="?page=feed&view=<?= $post["id"] ?>" class="flex items-center gap-1 text-sm text-twitch-muted" style="text-decoration:none;">
                                    <i class="fas fa-comment"></i>
                                    <span><?= $comments ?> <?= $comments === 1 ? "comment" : "comments" ?></span>
                                </a>

                                <?php if ($post["user_id"] === $_SESSION["user_id"]): ?>
                                    <a href="?page=feed&delete=<?= $post["id"] ?>&confirm=1" class="flex items-center gap-1 text-sm text-twitch-muted" style="text-decoration:none;" onclick="return confirm('Delete this post?')">
                            ...
                            <?php if ($view_post_id > 0): ?>
                                <?php
                                $stmt = $pdo->prepare("SELECT fc.*, u.username FROM feed_comments fc JOIN users u ON fc.user_id = u.id WHERE fc.post_id = ? ORDER BY fc.created_at ASC");
                                $stmt->execute([$post["id"]]);
                                $comments_list = $stmt->fetchAll();
                                ?>
                                <div class="mt-4 pt-4" style="border-top:1px solid #2D2D35;">
                                    <h4 class="font-bold text-sm mb-3">Comments (<?= count($comments_list) ?>)</h4>

                                    <?php if (!empty($comments_list)): ?>
                                        <?php foreach ($comments_list as $comment): ?>
                                            <div class="flex items-start gap-3 mb-3">
                                                <div class="tw-avatar" style="width:28px; height:28px; font-size:10px;"><?= get_avatar_letter($comment["username"]) ?></div>
                                                <div style="flex:1;">
                                                    <div class="flex items-center gap-2 mb-1">
                                                        <span class="font-medium text-xs"><?= htmlspecialchars($comment["username"]) ?></span>
                                                        <span class="text-xs text-twitch-muted"><?= time_ago($comment["created_at"]) ?></span>
                                                    </div>
                                                    <p class="text-sm"><?= htmlspecialchars($comment["content"]) ?></p>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
        ...
        <?php if ($total_pages > 1 && !$view_post_id): ?>
            <div class="flex items-center justify-center gap-2 mt-6">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="?page=feed&p=<?= $i ?><?= !empty($filter_type) ? "&type=" . urlencode($filter_type) : "" ?>" class="tw-btn <?= $i === $page_num ? "tw-btn-primary" : "tw-btn-ghost" ?> tw-btn-sm">
                        <?= $i ?>
                    </a>
                <?php endfor; ?>
            </div>
